...
public class Game {
  ...
  /**
   * Game constructor. Receives a list of players, a forward model and a game state. Sets unique and
   * final IDs to all players in the game, and performs initialisation of the game state and forward
   * model objects.
   *
  ...
  public Game(
      ...
      AbstractForwardModel realModel,
  ...
  /**
   * Game constructor. Receives a forward model and a game state. Performs initialisation of the
   * game state and forward model objects.
   *
   ...
   * @param randomizeParameters - if true, parameters are randomized for each run of each game (if
   *     possible).
   * @return - game instance created for the run
   */
  public static Game runOne(
      GameType gameToPlay,
      String parameterConfigFile,
      List<AbstractPlayer> players,
      long seed,
      boolean randomizeParameters,
      List<IGameListener> listeners,
      ActionController ac,
      int turnPause) {
    ...
    if (parameterConfigFile != null) {
      AbstractParameters params =
          AbstractParameters.createFromFile(gameToPlay, parameterConfigFile);
      game = gameToPlay.createGameInstance(players.size(), seed, params);
    } else game = gameToPlay.createGameInstance(players.size(), seed);
    if (game == null) System.out.println("Error game: " + gameToPlay);
    ...
    if (ac != null) {
      ...
      Timer guiUpdater =
          new Timer(
              (int) game.getCoreParameters().frameSleepMS, event -> game.updateGUI(gui, frame));
      guiUpdater.start();
   ...
   * @param randomizeParameters - if true, game parameters are randomized for each run of each game
   *     (if possible).
   * @param detailedStatistics - if true, detailed statistics are printed, otherwise just average of
   *     wins
   */
  public static void runMany(
      List<GameType> gamesToPlay,
      List<AbstractPlayer> players,
      Long seed,
      int nRepetitions,
      boolean randomizeParameters,
    ...
    for (GameType gt : gamesToPlay) {
      ...
      for (int i = 0; i < nPlayers; i++) {
        statSummaries[i] =
            new TAGNumericStatSummary("{Game: " + gt.name() + "; Player: " + agentNames[i] + "}");
      }
      ...
      if (game != null) {
        ...
        for (int i = 0; i < nPlayers; i++) {
          ...
          if (detailedStatistics) {
            ...
            System.out.println(
                statSummaries[i].name
                    + ": "
                    + statSummaries[i].mean()
                    + " (n="
                    + statSummaries[i].n()
                    + ")");
          }
  ...
  public static void runMany(
    ...
    for (GameType gt : gamesToPlay) {
      ...
      for (int i = 0; i < nRepetitions; i++) {
        Game game =
            runOne(gt, null, players, seeds[i], randomizeParameters, listeners, null, turnPause);
        if (game != null) {
          recordPlayerResults(statSummaries, game);
        }
      }
  ...
  /**
   * Records statistics of given game into the given StatSummary objects. Only WIN, LOSE or DRAW are
   * valid results recorded.
   *
  ...
  public static void recordPlayerResults(TAGNumericStatSummary[] statSummaries, Game game) {
    ...
    for (int p = 0; p < nPlayers; p++) {
      if (results[p] == CoreConstants.GameResult.WIN_GAME
          || results[p] == CoreConstants.GameResult.LOSE_GAME
   ...
   *
   * <p>Test class used to run a specific game. The user must specify: 1. Action controller for GUI
   * interactions / null for no visuals 2. Random seed for the game 3. Players for the game 4. Game
   * parameter configuration 5. Mode of running and then run this class.
   */
  public static void main(String[] args) {
    ...
    //        runMany(new ArrayList<GameType>() {{add(Uno);}}, players, 100L, 100, false, false,
    // null, turnPause);
  }
   ...
   * assigns players and their IDs, and initialises all players.
   *
   * @param players - new players for the game
   * @param newRandomSeed - random seed is updated in the game parameters object and used throughout
   *     the game.
   */
  ...
  public final void run() {
    ...
    while (gameState.isNotTerminal() && !stop) {
      ...
      synchronized (this) {
        ...
        boolean reacting =
            (gameState instanceof AbstractGameStateWithTurnOrder
                && ((AbstractGameStateWithTurnOrder) gameState).getTurnOrder()
                    instanceof ReactiveTurnOrder
  ...
  public final AbstractAction oneAction() {
    ...
    // we pause before each action is taken if running with a delay (e.g. for video recording with
    // random players)
    if (turnPause > 0)
      synchronized (this) {
        try {
    ...
    // copying the gamestate also copies the game parameters and resets the random seed (so agents
    // cannot use this
    // to reconstruct the starting hands etc.)
    AbstractGameState observation = gameState.copy(activePlayer);
    copyTime = (System.nanoTime() - s);
    //      System.out.printf("Total copyTime in ms = %.2f at tick %d (Avg %.3f) %n", copyTime /
    // 1e6, tick, copyTime / (tick +1.0) / 1e6);

    // Get actions for the player
    s = System.nanoTime();
    List<AbstractAction> observedActions =
    ...
    if (observedActions.size() == 0) {
      ...
      throw new AssertionError(
          "No actions available for player "
              + activePlayer
              + (lastAction != null
                  ? ". Last action: "
    ...
    // Either ask player which action to use or, in case no actions are available, report the
    // updated observation
    AbstractAction action = null;
    if (observedActions.size() > 0) {
      if (observedActions.size() == 1
          && (!(currentPlayer instanceof HumanGUIPlayer
                  || currentPlayer instanceof HumanConsolePlayer)
        ...
        if (debug)
          System.out.printf(
              "Game: %2d Tick: %3d\t%s%n",
              gameState.getGameID(), getTick(), action.getString(gameState));

        agentTime += (System.nanoTime() - s);
        nDecisions++;
      }
      if (gameState.coreGameParameters.competitionMode
          && action != null
          && !observedActions.contains(action)) {
        System.out.printf(
            "Action played that was not in the list of available actions: %s%n",
            action.getString(gameState));
        action = null;
      }
      // We publish an ACTION_CHOSEN message before we implement the action, so that observers can
      // record the state that led to the decision
      AbstractAction finalAction = action;
      listeners.forEach(
          l ->
              l.onEvent(
                  Event.createEvent(
                      Event.GameEvent.ACTION_CHOSEN, gameState, finalAction, activePlayer)));
    } else {
      currentPlayer.registerUpdatedObservation(observation);
    }
    ...
    if (action == null) throw new AssertionError("We have a NULL action in the Game loop");

    // Check player timeout
    if (observation.playerTimer[activePlayer].exceededMaxTime()) {
      action =
          forwardModel.disqualifyOrRandomAction(
              gameState.coreGameParameters.disqualifyPlayerOnTimeout, gameState);
    } else {
    ...
    // We publish an ACTION_TAKEN message once the action is taken so that observers can record the
    ...
    listeners.forEach(
        l ->
            l.onEvent(
                Event.createEvent(
                    Event.GameEvent.ACTION_TAKEN, gameState, finalAction1.copy(), activePlayer)));
    ...
    if (gameState.coreGameParameters.recordEventHistory) {
      ...
      for (int i = 0; i < gameState.getNPlayers(); i++) {
        gameState.recordHistory(
            String.format(
                "Player %d finishes at position %d with score: %.0f",
                i, gameState.getOrdinalPosition(i), gameState.getGameScore(i)));
      }
    }
    if (gameState.coreGameParameters.verbose) {
  ...
  /**
   * Retrieves the copy timer value, i.e. how long the game state took to produce player
  ...
  /**
   * Retrieves the next timer value, i.e. how long the forward model took to advance the game state
   * with an action.
   *
   * @return - next time
   */
  ...
  /**
   * Retrieves the action compute timer value, i.e. how long the forward model took to compute the
   * available actions in a game state.
   *
