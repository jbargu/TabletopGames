...
/**
 * This is useful where we have one action that requires a single follow-on action by the same
 * player that we are 100% confident *must* be the next action taken in the game.
 *
 * <p>This is a wrapper around a function that computes the available actions for the next step in
 * the sequence.
 */
public class OneShotExtendedAction extends AbstractAction implements IExtendedSequence {
  ...
  public OneShotExtendedAction(
      String name, Function<AbstractGameState, List<AbstractAction>> computeAvailableActions) {
  ...
  public boolean equals(Object obj) {
    if (obj instanceof OneShotExtendedAction) {
      ...
      return player == other.player
          ...
          && actionFunction.equals(other.actionFunction);
    }
    return false;
  }
